...
exports.sginUp = async (req, res, next) => {
    try {
        ...
        successResponse(res, { statusCode: 200, message: "registraion complited", data: user })
    } catch (error) {
        next(error)
    }
}
...
exports.login = async (req, res, next) => {
    try {
        ...
        const token = await user.createToken({ id: user._id, role: user.role, isActive: user.isActive }, process.env.SECRET_KEY, "1d")
        if (!token) {
            throw createError(400, "token not generated")
        }
        successResponse(res, { statusCode: 200, message: "login success", data: { token, user } })
